...
func showCompute(ctx context.Context, cfg vngcloud.Config, outputs *sdkOutputStore) {
	...
	record(outputs, cfg, "server/instance", "servers", servers, err)
	...
	record(outputs, cfg, "server/ssh_key", "ssh keys", keys, err)
	...
	record(outputs, cfg, "server/placement_group", "placement groups", groupItems, err)
	...
	record(outputs, cfg, "server/instance_security_group", "server security groups", secgroupItems, err)
	...
	record(outputs, cfg, "server/placement_group_member", "placement group members", memberItems, err)
	...
	record(outputs, cfg, "server/placement_group_policy", "placement group policies", policyItems, err)
	...
	record(outputs, cfg, "server/system_image_os", "os images", osImageItems, err)
	...
	record(outputs, cfg, "server/system_image_gpu", "gpu images", gpuImageItems, err)
	...
	record(outputs, cfg, "server/user_image", "user images", userImages, err)
}
